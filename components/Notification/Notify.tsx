...
const Notify = ({data}:Notify) => {
  ...
  <View className='mt-8 py-2 w-[100%] mx-auto border border-gray-400 rounded-md mb-3' key={notification.id} style={{ backgroundColor: theme ? '#152955' : 'white'}}>
    ...
    <View>
    <Text className='font-semibold text-base' style={{ color: textTheme }}> {notification.title} </Text>
    <Text style={{ color: textTheme }} className=''> {notification.message} </Text>
    </View>
    
    </View>

      
      <Text className='mb-4 mr-2 text-[#808080]' style={{ color: textTheme }}>{getTimeAgo(notification.created_at)}</Text>
