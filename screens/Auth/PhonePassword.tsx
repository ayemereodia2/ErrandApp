...
export default function PhonePassword({ navigation }: any) {
  ...
  const submitPhone = async (data: IData) => {
    try {
      const phoneNumber = `+234${phone.substring(1)}`
      console.log(">>>>>phone from verify", phoneNumber);
      
      await AsyncStorage.setItem('phone', phoneNumber)
      const _rs = await _fetch({
        method: 'POST',
        _url: `/user/verify-phone`,
        body: { intent:'forgot_pass', phone_number: phoneNumber },
      })
        ...
        <View className="px-4">
          ...
          <View className="text-[#333333] font-inter py-4 space-y-1">
            ...
            <View className="pt-2 space-y-6">
              ...
              <View className="">
                <Text className="mb-2 text-[#243763] text-[14px]">
                  Phone Number
                </Text>
                <TextInput
                  value={phone}
